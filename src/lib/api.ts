...
export async function sendMessage(
  ...
  if (!API_BASE) {
    return `[Demo] Has dicho: «${message}». Cuando conectes el backend (astronomIA o n8n), la respuesta real aparecerá aquí. Configura VITE_API_URL en .env para llamar a la API.`;
  }
  ...
  if (!res.ok) {
    ...
    return `Error del servidor: ${res.status}. ${text.slice(0, 200)}`;
  }

  const data = (await res.json()) as AnalyzeResponse;
  if (data.status === "error") {
    return data.summary || "El análisis no pudo completarse.";
  }
  return data.summary;
}
